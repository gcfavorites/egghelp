...
#---
	...
	proc msg:horoscope {unick uhost handle str} {pub:horoscope $unick $uhost $handle $unick $str ; return}
	...
	proc pub:horoscope {unick uhost handle uchan str} {
		...
		if {$uchan ne $unick} {if {![channel get $uchan $chflag] ^ $flagactas eq "no"} {return}}

		set why  [queue_isfreefor $id]
		if {$why != ""} {lput puthelp $why [subst -nocommands $errsend] ; return}
	...
	if {$htype == 1} {
		...
		if {$logrequests ne ""} {set logstr [subst -nocommands $logrequests] ; lput putlog $logstr "$unamespace: $furl : "}

		if {[queue_add "$furl" $id "[namespace current]::horo:parser" [list $unick $uhost $wdayname $uchan $wlove]]} {
	...
	proc horo:parser {errid errstr body extra} {
		...
		if {$htype} {
			if {[regexp -nocase -- {<h2>(.+?)</h2>.*?<!--text begin-->(.+?)<!--text end-->.*?<h2>(.+?)</h2>.*?<!--r.daily.tom._file_.text-->(.+?)</div>} $str -> lovesign lovehoro gensign genhoro]} {
				if {$ustr eq "1"} {lput putserv "\00305$lovesign\003 \002::\002 [sspace $lovehoro]" $prefix} {lput putserv "\00305$gensign\003  \002::\002 [sspace $genhoro]" $prefix}} {lput putserv "\037Ошибка парсинга\037." [subst -nocommands $errsend]}
		} {
			set data $str
			regsub -all -- \n $data {} data
			...
			regsub -all -- {<br />} $data "" data
   				foreach item [split $data \n] {
   					if {[regexp -- {<div\ class=.*?\ id=.*?><h2\ class=.*?\ id=\"inl\">(.*?)</h2><h2 class=\"inl\">(.*?)</h2><h3 class=\"cat\" id=.*?>(.*?)</h3>} $item g chislo horolove2 horoobsh]} {lput putserv "\00310 [sspace $chislo] $horolove2 \003:: \00303 Раздел: $horoobsh\003" $prefix}
   					if {[regexp -- {<h4>(.*?)<span class=.*?>(.*?)</span></h4></div><p>(.*?)</p>} $item g q w e]} {lput putserv "\00303 $q $w\003 :: $e" $prefix ; return}
	...
	proc cmdaliases {{action {bind}}} {
		foreach {bindtype} {pub msg dcc} {
			foreach {bindproc} [info vars "[namespace current]::${bindtype}:*"] {
				...
				foreach {alias} [set $bindproc] {catch {$action $bindtype -|- [set ${bindtype}prefix]$alias $bindproc}}				
			}
		}
