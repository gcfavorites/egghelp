...
#----------------------------------------------------------------------------
	...
	variable pub:egghelp		"$unamespace script скрипт"
...
#---параметры
	...
	if {[regexp -nocase -- {^-(\d+)} $ustr -> mpg]} {set mpage $mpg ; regsub -- {-\d+\s+} $ustr "" ustr} {set mpage 1}
		...
		lput puthelp "\002Формат\002: $pubprefix\egghelp \[-число\] <file>" $prefix		
		lput puthelp "Поиск скриптов на egghelp.ru. \002Пример:\002 $pubprefix\egghelp oneday" $prefix		
		return
		}
		...
		set query [::http::formatQuery id "" string $ustr form "Начать поиск" limit $maxres]
		...
		set cnt 0 ; set eout "" ; set inf ""
			foreach r [split $res \n] {
				if {[regexp -- {<a href="file.php\?id=(.*?)">(.*?)</a>.*?</a><br />(.*?)</td>\s+(.*?)<br /><b>(.*?)</b></td>} $r -> id name desc inf author]} {
					...
					if {$cnt == $mpage} {set eout "\002$name\002 :: $author :: \00305[sspace $desc]\003 :: \[ v.[string trim [lindex $inf 1]] / Suzi: [string map {" " "-"} [lindex $inf 0]] / Загр:[lindex $inf 4] / Разм:[lindex $inf 2] / Дата:[lindex $inf 5] \] @ \00312\037http://www.egghelp.ru/file.php?id=$id\003\037"}
				}
			}
		if {$cnt == 0} {lput putserv "\037Ничего не найдено\037." $prefix; return} 
		if {$cnt >= $mpage} {lput putserv "\[$mpage/$cnt\] :: [string map {"&amp;" "&"} $eout]" $prefix} {lput putserv "\037Неверный параметр\037 (\-$mpage) - всего найдено: $cnt" $prefix}
