...
#----------------------------------------------------------------------------
	...
	variable pub:fext		"$unamespace ext рас"
...
#---параметры
	set ustr [tolow $str]
		...
		lput puthelp "\002Формат\002: !ext <file extension>" $prefix		
		lput puthelp "информация о расширении файла (на английском). \002Пример:\002 !ext jpg" $prefix		
		return
		}
...
#----------------------------------------------------------------------------
	...
	if {([regexp {<meta name="description" content=".*?There are no results in the database for this extension.*?">} $str]) || ([regexp {<title>Oops - Something Went Wrong - FILExt</title>} $str])} {
		lput putserv "Filext: \037ничего не найдено\037." $prefix
		return
	}
